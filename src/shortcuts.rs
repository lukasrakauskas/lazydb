...
const fn ch(c: char, d: &'static str) -> KeyPattern {
    KeyPattern { code: KeyCode::Char(c), require: 0, forbid: CTRL | ALT, display: d }
}
/// Ctrl+char.
const fn ctrl(c: char, d: &'static str) -> KeyPattern {
    KeyPattern { code: KeyCode::Char(c), require: CTRL, forbid: 0, display: d }
}
/// Shift+<special key> (arrows), no Ctrl/Alt.
const fn shift(code: KeyCode, d: &'static str) -> KeyPattern {
    KeyPattern { code, require: SHIFT, forbid: CTRL | ALT, display: d }
}
/// Alt+<special key>.
const fn alt(code: KeyCode, d: &'static str) -> KeyPattern {
    KeyPattern { code, require: ALT, forbid: 0, display: d }
}
...
const fn bare(code: KeyCode, d: &'static str) -> KeyPattern {
    KeyPattern { code, require: 0, forbid: 0, display: d }
}
...
impl KeyPattern {
    pub fn matches(&self, key: &KeyEvent) -> bool {
        ...
        key.code == self.code
            && key.modifiers.contains(req)
            && !key.modifiers.intersects(forb)
    }
}
...
pub static GLOBAL: &[Binding] = &[
    Binding { keys: &[ctrl('c', "Ctrl+C"), ctrl('q', "Ctrl+Q")], label: "quit", action: Action::Quit, hidden: false },
    Binding { keys: &[ctrl('r', "Ctrl+R"), bare(KeyCode::F(5), "F5"), alt(KeyCode::Enter, "Opt+Enter")], label: "run", action: Action::RunQuery, hidden: false },
];
...
pub static COMMON_PANE: &[Binding] = &[
    Binding { keys: &[bare(KeyCode::Tab, "Tab")], label: "focus", action: Action::FocusNext, hidden: false },
    Binding { keys: &[ch('1', "1")], label: "conn", action: Action::FocusConnections, hidden: true },
    Binding { keys: &[ch('2', "2")], label: "editor", action: Action::FocusEditor, hidden: true },
    Binding { keys: &[ch('3', "3")], label: "results", action: Action::FocusResults, hidden: true },
    Binding { keys: &[ch('4', "4")], label: "schema", action: Action::FocusSchema, hidden: true },
    Binding { keys: &[ch('?', "?")], label: "key-log", action: Action::ToggleKeyLog, hidden: false },
    Binding { keys: &[ch('f', "f")], label: "features", action: Action::ToggleFeatures, hidden: false },
    Binding { keys: &[ch('q', "q")], label: "quit", action: Action::Quit, hidden: false },
];

static CONNECTIONS: &[Binding] = &[
    Binding { keys: &[ch('j', "j"), bare(KeyCode::Down, "↓")], label: "down", action: Action::MoveDown, hidden: false },
    Binding { keys: &[ch('k', "k"), bare(KeyCode::Up, "↑")], label: "up", action: Action::MoveUp, hidden: false },
    Binding { keys: &[bare(KeyCode::Enter, "⏎")], label: "connect", action: Action::ConnectSelected, hidden: false },
    Binding { keys: &[ch('n', "n")], label: "new", action: Action::NewConnection, hidden: false },
    Binding { keys: &[ch('d', "d")], label: "delete", action: Action::DeleteConnection, hidden: false },
];

static EDITOR: &[Binding] = &[
    // specific-first: Shift+Up/Down (history recall) before plain Up/Down.
    Binding { keys: &[shift(KeyCode::Up, "Shift+↑")], label: "hist-prev", action: Action::RecallHistoryOlder, hidden: false },
    Binding { keys: &[shift(KeyCode::Down, "Shift+↓")], label: "hist-next", action: Action::RecallHistoryNewer, hidden: false },
    Binding { keys: &[bare(KeyCode::Up, "↑")], label: "up", action: Action::EditorUp, hidden: false },
    Binding { keys: &[bare(KeyCode::Down, "↓")], label: "down", action: Action::EditorDown, hidden: false },
    Binding { keys: &[bare(KeyCode::Left, "←")], label: "left", action: Action::EditorLeft, hidden: false },
    Binding { keys: &[bare(KeyCode::Right, "→")], label: "right", action: Action::EditorRight, hidden: false },
    Binding { keys: &[bare(KeyCode::Home, "Home")], label: "home", action: Action::EditorHome, hidden: false },
    Binding { keys: &[bare(KeyCode::End, "End")], label: "end", action: Action::EditorEnd, hidden: false },
    Binding { keys: &[bare(KeyCode::Enter, "⏎")], label: "newline", action: Action::EditorNewline, hidden: false },
    Binding { keys: &[bare(KeyCode::Backspace, "⌫")], label: "del", action: Action::EditorBackspace, hidden: false },
    Binding { keys: &[bare(KeyCode::Tab, "Tab")], label: "focus", action: Action::FocusNext, hidden: false },
];
...
static EDITOR_AUTOCOMPLETE: &[Binding] = &[
    Binding { keys: &[shift(KeyCode::Up, "Shift+↑")], label: "hist-prev", action: Action::RecallHistoryOlder, hidden: false },
    Binding { keys: &[shift(KeyCode::Down, "Shift+↓")], label: "hist-next", action: Action::RecallHistoryNewer, hidden: false },
    Binding { keys: &[bare(KeyCode::Tab, "Tab"), bare(KeyCode::Enter, "⏎")], label: "accept", action: Action::AcceptCompletion, hidden: false },
    Binding { keys: &[bare(KeyCode::Down, "↓")], label: "next", action: Action::CompletionNext, hidden: false },
    Binding { keys: &[bare(KeyCode::Up, "↑")], label: "prev", action: Action::CompletionPrev, hidden: false },
    Binding { keys: &[bare(KeyCode::Esc, "Esc")], label: "dismiss", action: Action::DismissCompletion, hidden: false },
    Binding { keys: &[bare(KeyCode::Left, "←")], label: "left", action: Action::EditorLeft, hidden: false },
    Binding { keys: &[bare(KeyCode::Right, "→")], label: "right", action: Action::EditorRight, hidden: false },
    Binding { keys: &[bare(KeyCode::Home, "Home")], label: "home", action: Action::EditorHome, hidden: false },
    Binding { keys: &[bare(KeyCode::End, "End")], label: "end", action: Action::EditorEnd, hidden: false },
...
static RESULTS: &[Binding] = &[
    Binding { keys: &[ch('j', "j"), bare(KeyCode::Down, "↓")], label: "row↓", action: Action::MoveDown, hidden: false },
    Binding { keys: &[ch('k', "k"), bare(KeyCode::Up, "↑")], label: "row↑", action: Action::MoveUp, hidden: false },
    Binding { keys: &[ch('l', "l"), bare(KeyCode::Right, "→")], label: "col→", action: Action::MoveRight, hidden: false },
    Binding { keys: &[ch('h', "h"), bare(KeyCode::Left, "←")], label: "col←", action: Action::MoveLeft, hidden: false },
    Binding { keys: &[bare(KeyCode::PageDown, "PgDn")], label: "pg↓", action: Action::PageDown, hidden: false },
    Binding { keys: &[bare(KeyCode::PageUp, "PgUp")], label: "pg↑", action: Action::PageUp, hidden: false },
...
static SCHEMA: &[Binding] = &[
    Binding { keys: &[ch('j', "j"), bare(KeyCode::Down, "↓")], label: "down", action: Action::MoveDown, hidden: false },
    Binding { keys: &[ch('k', "k"), bare(KeyCode::Up, "↑")], label: "up", action: Action::MoveUp, hidden: false },
    Binding { keys: &[bare(KeyCode::Enter, "⏎"), ch('l', "l"), bare(KeyCode::Right, "→")], label: "expand", action: Action::SchemaExpand, hidden: false },
    Binding { keys: &[ch('h', "h"), bare(KeyCode::Left, "←")], label: "collapse", action: Action::SchemaCollapse, hidden: false },
];

static FORM: &[Binding] = &[
    Binding { keys: &[bare(KeyCode::Esc, "Esc")], label: "cancel", action: Action::FormCancel, hidden: false },
    Binding { keys: &[bare(KeyCode::Enter, "⏎")], label: "save", action: Action::FormSave, hidden: false },
    Binding { keys: &[bare(KeyCode::Tab, "Tab"), bare(KeyCode::Down, "↓")], label: "next", action: Action::FormFieldNext, hidden: false },
    Binding { keys: &[bare(KeyCode::BackTab, "Shift+Tab"), bare(KeyCode::Up, "↑")], label: "prev", action: Action::FormFieldPrev, hidden: false },
    Binding { keys: &[bare(KeyCode::Left, "←")], label: "left", action: Action::FormFieldLeft, hidden: false },
    Binding { keys: &[bare(KeyCode::Right, "→")], label: "right", action: Action::FormFieldRight, hidden: false },
    Binding { keys: &[bare(KeyCode::Home, "Home")], label: "home", action: Action::FormFieldHome, hidden: false },
    Binding { keys: &[bare(KeyCode::End, "End")], label: "end", action: Action::FormFieldEnd, hidden: false },
    Binding { keys: &[bare(KeyCode::Backspace, "⌫")], label: "del", action: Action::FormFieldBackspace, hidden: false },
];

static FEATURES: &[Binding] = &[
    Binding { keys: &[bare(KeyCode::Esc, "Esc"), ch('f', "f"), ch('q', "q")], label: "close", action: Action::FeaturesClose, hidden: false },
    Binding { keys: &[ch('j', "j"), bare(KeyCode::Down, "↓")], label: "down", action: Action::FeaturesNext, hidden: false },
    Binding { keys: &[ch('k', "k"), bare(KeyCode::Up, "↑")], label: "up", action: Action::FeaturesPrev, hidden: false },
    Binding { keys: &[ch(' ', "Space"), bare(KeyCode::Enter, "⏎")], label: "toggle", action: Action::FeaturesToggle, hidden: false },
];

static CONFIRM: &[Binding] = &[
    Binding { keys: &[ch('y', "y"), ch('Y', "Y")], label: "confirm", action: Action::ConfirmYes, hidden: false },
...
mod tests {
    ...
    fn results_view_binds_nav_copy_and_chrome() {
        let v = View::Results;
        assert_eq!(match_key(v, k(KeyCode::Char('y'), KeyModifiers::NONE)), Some(Action::CopyRowJson));
        // Ctrl+y must NOT copy a row (plain-char bindings forbid Ctrl).
        assert_eq!(match_key(v, k(KeyCode::Char('y'), KeyModifiers::CONTROL)), None);
        assert_eq!(match_key(v, k(KeyCode::Char('s'), KeyModifiers::CONTROL)), Some(Action::CopyResultCsv));
        assert_eq!(match_key(v, k(KeyCode::Char('j'), KeyModifiers::NONE)), Some(Action::MoveDown));
        // common pane chrome
        assert_eq!(match_key(v, k(KeyCode::Char('1'), KeyModifiers::NONE)), Some(Action::FocusConnections));
        assert_eq!(match_key(v, k(KeyCode::Char('q'), KeyModifiers::NONE)), Some(Action::Quit));
        // global
        assert_eq!(match_key(v, k(KeyCode::Char('q'), KeyModifiers::CONTROL)), Some(Action::Quit));
        assert_eq!(match_key(v, k(KeyCode::F(5), KeyModifiers::NONE)), Some(Action::RunQuery));
        // Tab → focus next (common), not a result nav key
        assert_eq!(match_key(v, k(KeyCode::Tab, KeyModifiers::NONE)), Some(Action::FocusNext));
        // unbound plain char falls through
        assert_eq!(match_key(v, k(KeyCode::Char('z'), KeyModifiers::NONE)), None);
    }

    #[test]
    fn editor_shift_arrows_win_over_plain_arrows() {
        let v = View::Editor;
        assert_eq!(match_key(v, k(KeyCode::Up, KeyModifiers::SHIFT)), Some(Action::RecallHistoryOlder));
        assert_eq!(match_key(v, k(KeyCode::Down, KeyModifiers::SHIFT)), Some(Action::RecallHistoryNewer));
        // plain arrows are editor cursor moves, not history recall
        assert_eq!(match_key(v, k(KeyCode::Up, KeyModifiers::NONE)), Some(Action::EditorUp));
        assert_eq!(match_key(v, k(KeyCode::Down, KeyModifiers::NONE)), Some(Action::EditorDown));
        // a typed letter is NOT a shortcut (raw text input fall-through)
        assert_eq!(match_key(v, k(KeyCode::Char('a'), KeyModifiers::NONE)), None);
        // global run still reaches the editor
        assert_eq!(match_key(v, k(KeyCode::Char('r'), KeyModifiers::CONTROL)), Some(Action::RunQuery));
        // Tab cycles focus from the editor (editor-specific, not common)
        assert_eq!(match_key(v, k(KeyCode::Tab, KeyModifiers::NONE)), Some(Action::FocusNext));
        // digits are NOT bound in the editor (they type)
        assert_eq!(match_key(v, k(KeyCode::Char('2'), KeyModifiers::NONE)), None);
    }

    #[test]
    fn autocomplete_overrides_tab_and_arrows() {
        let v = View::EditorAutocomplete;
        assert_eq!(match_key(v, k(KeyCode::Tab, KeyModifiers::NONE)), Some(Action::AcceptCompletion));
        assert_eq!(match_key(v, k(KeyCode::Enter, KeyModifiers::NONE)), Some(Action::AcceptCompletion));
        assert_eq!(match_key(v, k(KeyCode::Down, KeyModifiers::NONE)), Some(Action::CompletionNext));
        assert_eq!(match_key(v, k(KeyCode::Esc, KeyModifiers::NONE)), Some(Action::DismissCompletion));
    ...
    fn features_modal_q_closes_and_ctrl_q_quits() {
        let v = View::Features;
        assert_eq!(match_key(v, k(KeyCode::Char('q'), KeyModifiers::NONE)), Some(Action::FeaturesClose));
        assert_eq!(match_key(v, k(KeyCode::Char('f'), KeyModifiers::NONE)), Some(Action::FeaturesClose));
        assert_eq!(match_key(v, k(KeyCode::Char(' '), KeyModifiers::NONE)), Some(Action::FeaturesToggle));
        ...
        assert_ne!(match_key(v, k(KeyCode::Char('q'), KeyModifiers::NONE)), Some(Action::Quit));
    }

    #[test]
    fn schema_enter_l_right_expand_and_h_left_collapse() {
        let v = View::Schema;
        assert_eq!(match_key(v, k(KeyCode::Enter, KeyModifiers::NONE)), Some(Action::SchemaExpand));
        assert_eq!(match_key(v, k(KeyCode::Char('l'), KeyModifiers::NONE)), Some(Action::SchemaExpand));
        assert_eq!(match_key(v, k(KeyCode::Right, KeyModifiers::NONE)), Some(Action::SchemaExpand));
        assert_eq!(match_key(v, k(KeyCode::Char('h'), KeyModifiers::NONE)), Some(Action::SchemaCollapse));
        assert_eq!(match_key(v, k(KeyCode::Left, KeyModifiers::NONE)), Some(Action::SchemaCollapse));
    }

    #[test]
    fn form_tab_and_down_both_advance_field() {
        let v = View::Form;
        assert_eq!(match_key(v, k(KeyCode::Tab, KeyModifiers::NONE)), Some(Action::FormFieldNext));
        assert_eq!(match_key(v, k(KeyCode::Down, KeyModifiers::NONE)), Some(Action::FormFieldNext));
        assert_eq!(match_key(v, k(KeyCode::BackTab, KeyModifiers::NONE)), Some(Action::FormFieldPrev));
        assert_eq!(match_key(v, k(KeyCode::Enter, KeyModifiers::NONE)), Some(Action::FormSave));
        assert_eq!(match_key(v, k(KeyCode::Esc, KeyModifiers::NONE)), Some(Action::FormCancel));
    ...
    fn confirm_modal_y_n() {
        ...
        assert_eq!(match_key(v, k(KeyCode::Char('y'), KeyModifiers::NONE)), Some(Action::ConfirmYes));
        assert_eq!(match_key(v, k(KeyCode::Char('Y'), KeyModifiers::SHIFT)), Some(Action::ConfirmYes));
        assert_eq!(match_key(v, k(KeyCode::Char('n'), KeyModifiers::NONE)), Some(Action::ConfirmNo));
        assert_eq!(match_key(v, k(KeyCode::Esc, KeyModifiers::NONE)), Some(Action::ConfirmNo));
    }
    ...
    fn active_results_includes_global_run_and_view_copy() {
        ...
        assert!(actions.contains(&Action::CopyRowJson), "missing copy-row in results bar");
        assert!(actions.contains(&Action::RunQuery), "missing global run in results bar");
        assert!(actions.contains(&Action::Quit), "missing global quit in results bar");
        assert!(actions.contains(&Action::FocusConnections), "missing common pane chrome");
    }
    ...
    fn bar_hides_pane_focus_but_keys_still_match() {
        ...
        assert!(!bar.contains(&Action::FocusConnections), "1 should be hidden from bar");
        assert!(!bar.contains(&Action::FocusEditor), "2 should be hidden from bar");
        assert!(!bar.contains(&Action::FocusResults), "3 should be hidden from bar");
        assert!(!bar.contains(&Action::FocusSchema), "4 should be hidden from bar");
        // ...yet the keymap still routes them (common pane chrome is active
        // in the non-editor panes):
        assert_eq!(match_key(View::Results, k(KeyCode::Char('2'), KeyModifiers::NONE)), Some(Action::FocusEditor));
        // editor view: digits type, so 2 is NOT bound there.
        assert_eq!(match_key(View::Editor, k(KeyCode::Char('2'), KeyModifiers::NONE)), None);
    }

    #[test]
    fn keys_display_joins_with_slash() {
        // The run binding has 3 keys; the help bar shows them as one group.
        let run = GLOBAL.iter().find(|b| b.action == Action::RunQuery).unwrap();
